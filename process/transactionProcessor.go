...
func (tp *TransactionProcessor) getTxFromObserver(
	...
	if err != nil {
		log.Info("<getTxDebug> cannot get transaction", "address", observer.Address, "respCode", respCode, "error", err.Error())
...
func (tp *TransactionProcessor) getTxFromDestShard(txHash string, dstShardID uint32, withEvents bool) (*data.FullTransaction, bool) {
	...
	for _, dstObserver := range destinationShardObservers {
		...
		if err != nil {
			log.Info("<getTxDebug> cannot get transaction from dest shard", "address", dstObserver.Address, "tx hash", txHash, "shard", dstShardID, "error", err.Error())
			continue
		}
